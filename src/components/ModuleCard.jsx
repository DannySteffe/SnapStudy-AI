...
export default function ModuleCard({ module, onDelete, onRegenerate }) {
  ...
  const handleView = (e) => {
    // Prevent navigation if clicking on menu or buttons
      ...
      >
        <div className="flex justify-between items-start mb-4">
          ...
          <div className="relative z-10">
            ...
            <AnimatePresence>
              {isMenuOpen && (
                <motion.div
                  ...
                  animate={{ opacity: 1, scale: 1 }}
                  exit={{ opacity: 0, scale: 0.95 }}
                  className="absolute right-0 mt-1 w-40 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 py-1 z-20"
                  onClick={(e) => e.stopPropagation()}
                >
            <button 
              onClick={() => { navigate(`/module/${module._id || module.id}`); setIsMenuOpen(false); }}
            ...
            <button 
              onClick={() => { onRegenerate(module._id || module.id); setIsMenuOpen(false); }}
              className="w-full text-left px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 flex items-center gap-2"
            >
            ...
            <button 
              onClick={() => { onDelete(module._id || module.id); setIsMenuOpen(false); }}
        ...
        <div className="flex items-center text-xs text-slate-400 dark:text-slate-500 mt-auto">
